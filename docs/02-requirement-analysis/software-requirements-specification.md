...
[§5 Scope](../01-discovery-and-planning/discovery-and-planning.md#5-scope); this SRS covers the
...
- Trace shortcuts: **Charter §n** = charter section; **BS §n** = Build Spec v3 section;
  **UX-PB Tn** = UI/UX Execution Playbook task.
...
(e.g. GitHub Pages). See [Build Spec v3 §2](../specs/build-spec-v3.md).

### 2.2 User Classes & Characteristics

Per charter [§7 User Personas](../01-discovery-and-planning/discovery-and-planning.md#7-user-personas):
...
tech stack fixed by [Build Spec v3 §2](../specs/build-spec-v3.md) (Vite + React + TypeScript,
Tailwind, recharts, mermaid). See charter [§9](../01-discovery-and-planning/discovery-and-planning.md#9-assumptions-constraints--dependencies).
...
[§9](../01-discovery-and-planning/discovery-and-planning.md#9-assumptions-constraints--dependencies).
...
|---|---|---|---|
| FR-SHELL-1 | Provide two modes — **Guided** (plain language) and **Expert** (technical terms, editable weights) — switchable at runtime, with the choice persisted. | Must | Charter §7; BS §12 |
| FR-SHELL-2 | Provide an **ID/EN language toggle** that localizes every visible string at runtime (default ID). | Must | Charter §5; BS §2 |
| FR-SHELL-3 | Provide a **dark/light theme toggle**, both meeting WCAG AA contrast. | Must | Charter §5, §11 |
| FR-SHELL-4 | Display a **permanent, visible disclaimer** that scores are tunable heuristics, not facts. | Must | Charter §21 (R2) |
| FR-SHELL-5 | Offer **scenario presets** (startup MVP, regulated/enterprise, high-traffic e-commerce, IoT/streaming, internal tool) that populate all factors. | Must | BS §12; Charter §5 |
| FR-SHELL-6 | Provide **reset** of all answers, guarded by confirmation and reversible via undo. | Must | UX-PB T4 |
| FR-SHELL-7 | Provide a **glossary** of terms and contextual help/tooltips. | Must | Charter §5; UX-PB T9 |
| FR-SHELL-8 | Provide a **command palette and keyboard shortcuts** covering all core actions. | Should | UX-PB T2 |

### 3.2 Step 1 — Project Factors

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-FACT-1 | Present **≥12 project factors** (target 14), grouped, each with three ordinal levels (0–2). | Must | Charter §5; BS §4 |
| FR-FACT-2 | Show localized **help text** per factor (what it means + why it shifts QA priorities). | Must | BS §4 |
| FR-FACT-3 | On any factor change, **instantly update** QA priorities, all dimension rankings, charts, and analyses. | Must | BS §14.2 |
| FR-FACT-4 | Support **collapsible groups** and progressive disclosure of advanced factors. | Should | UX-PB T3, T9 |
| FR-FACT-5 | Apply documented **default factor levels** (all level 0 except time-to-market = 1). | Must | BS §12 |

### 3.3 Step 2 — Quality-Attribute Priorities

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-QA-1 | Derive QA weights from factors via the **factor→QA influence matrix** (contribution = influence × level; budget inverted), clamp negatives to 0, and **normalize to sum 100 %**. | Must | BS §5 |
| FR-QA-2 | Display the 12 QA weights as a ranked bar chart (the **utility tree**). | Must | BS §8 |
| FR-QA-3 | In Expert mode, allow **direct override** of any QA weight and **lock** it so factor changes do not overwrite it. | Should | BS §5 |
| FR-QA-4 | Visibly mark **economic QAs** (cost efficiency, time-to-market) as outside the ISO product-quality model. | Must | BS §3 |

### 3.4 Step 3 — Recommendation & Analysis

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-REC-1 | Recommend across **five orthogonal dimensions** (deployment, communication, data, code structure, frontend). | Must | BS §6 |
| FR-REC-2 | Compute each option's **composite score** and rank options within each dimension, displaying scores normalized to 0–100. | Must | BS §6, §8 |
| FR-REC-3 | Render a **radar chart** overlaying the top options across all 12 QAs. | Must | BS §8 |
| FR-REC-4 | Show a per-option **QA contribution breakdown** that reconciles exactly to the composite score. | Must | BS §8, §14.10 |
| FR-REC-5 | Provide a **comparison mode** for 2–3 options side-by-side. | Should | BS §8 |
| FR-REC-6 | Flag a **close call** when the top two options in a dimension are within 10 %. | Must | BS §8 |
| FR-REC-7 | Perform **sensitivity analysis**: name the single factor change (±1 level) that flips the D1 winner, or label the result **robust**. | Must | BS §8 |
| FR-REC-8 | Present a **risk register** for the chosen options (likelihood, impact, mitigation). | Must | BS §8, §9 |
| FR-REC-9 | Detect **anti-patterns** on the chosen combination via rules with severity (info/warning/danger). | Must | BS §10 |
| FR-REC-10 | Generate suggested, measurable **fitness functions** from the top-weighted QAs. | Should | BS §11 |
| FR-REC-11 | Show qualitative **cost & operational-complexity indicators** (Low/Med/High) per D1 option. | Should | BS §8 |
| FR-REC-12 | Given an optional **current architecture**, suggest an incremental **migration path** (Strangler Fig where legacy is heavy). | Should | BS §8 |
| FR-REC-13 | Provide a **"How scoring works" / methodology panel** citing ISO/IEC 25010:2023, ATAM, ADD, and fitness functions. | Must | BS §8; Charter §1 |

### 3.5 Step 4 — Outputs & Sharing

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-OUT-1 | Export an **ADR in MADR format**. | Must | BS §12 |
| FR-OUT-2 | Export a **full report** (Markdown + print stylesheet). | Must | BS §12 |
| FR-OUT-3 | Export **scores as CSV** and the **assessment as JSON**. | Should | BS §12 |
| FR-OUT-4 | Provide a **share-via-URL** link that round-trips to identical state. | Must | BS §14.14 |
| FR-OUT-5 | Render a **C4-style Mermaid diagram stub** reflecting the chosen D1 style. | Could | BS §12 |
| FR-OUT-6 | Support **import/export of a custom configuration JSON** for extensibility. | Should | BS §12 |

### 3.6 State, Persistence & Reproducibility

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-STATE-1 | Persist the full assessment state to **`localStorage`**. | Must | BS §2 |
| FR-STATE-2 | Encode the full state in the **URL hash** for shareable links. | Must | BS §2 |
| FR-STATE-3 | Record the **model version** in every result, export, and shared URL. | Must | Charter §15 (R8) |
| FR-STATE-4 | **Validate and sanitize** any state parsed from the URL before use. | Must | Charter §18 |

### 3.7 UI States & Feedback

| ID | The system shall… | Priority | Trace |
|---|---|---|---|
| FR-UI-1 | Show an always-visible **save-state indicator** (Saving / All changes saved). | Must | UX-PB T4 |
| FR-UI-2 | Use **skeleton loading** (not a full-screen spinner) while recomputing. | Must | UX-PB T1 |
| FR-UI-3 | Present **three-layer errors** (what / why / how to fix) with a retry and a copyable request ID for recoverable failures. | Must | UX-PB T5 |
| FR-UI-4 | Provide **undo** for destructive actions. | Must | UX-PB T4 |
| FR-UI-5 | Provide a **guiding empty state** with a "load sample data" action. | Must | UX-PB T9 |
| FR-UI-6 | Avoid layout shift and keep transitions within 150–250 ms. | Should | UX-PB T1 |
...
|---|---|---|---|---|
| NFR-PERF-1 | Reflect any factor change in priorities, charts, and rankings within **~100 ms perceived** latency on a mid-range device. | Must | UX-PB T1 | T |
| NFR-PERF-2 | Enable a **median time-to-first-recommendation ≤ 5 minutes** (KPI K3). | Must | Charter §4 (K3) | T |
| NFR-USE-1 | Achieve a **System Usability Scale ≥ 75** at beta (KPI K5). | Should | Charter §4 (K5) | T |
| NFR-USE-2 | Be usable **without mandatory setup** (presets and sample data available immediately). | Must | UX-PB T9 | D |
| NFR-USE-3 | Be **consistent and predictable**: honor standard shortcuts; one term per concept; consistent color meaning. | Must | UX-PB T6 | I |
| NFR-A11Y-1 | Meet **WCAG 2.2 AA** contrast in both themes. | Must | Charter §5 | T |
| NFR-A11Y-2 | Be **fully keyboard-operable**, with accessible names on all controls. | Must | BS §14.15 | T |
| NFR-I18N-1 | Resolve **every string from the i18n dictionary** with no untranslated keys when toggling language. | Must | BS §14.13 | T |
| NFR-REL-1 | Never lose user data **without a warning or undo**. | Must | Charter §11 | D |
| NFR-REL-2 | Keep **old shared URLs and exports readable** across releases (breaking changes only on MAJOR). | Should | Charter §15 | A |
| NFR-SEC-1 | Avoid `eval`/`dangerouslySetInnerHTML` on untrusted input and **sanitize URL-parsed state**. | Must | Charter §18 | I |
| NFR-SEC-2 | Use a **lockfile and dependency vulnerability scanning**; consider a Content Security Policy. | Should | Charter §18 | I |
| NFR-SEC-3 | Contain **no secrets** in the codebase. | Must | Charter §18 | I |
| NFR-PRIV-1 | Collect **no PII** and store data **only** in the browser and user-shared URL. | Must | Charter §17 | I |
| NFR-PRIV-2 | Include **no analytics by default**; any future analytics must be privacy-respecting, declared, and able to be disabled. | Must | Charter §17 | I |
| NFR-COMPAT-1 | Be **responsive to ≥ 360 px** and function on modern evergreen browsers. | Must | Charter §9 | T |
| NFR-COMPAT-2 | Be deployable as a **static site** (incl. a sub-path base, e.g. GitHub Pages). | Must | Deployment guide | D |
| NFR-MAINT-1 | Keep **all model values** (factors, weights, fit vectors, rules, strings) in `config/`/`i18n/`, never hard-coded. | Must | BS §13; Charter §14.7 | I |
| NFR-MAINT-2 | Provide **unit tests** for scoring, normalization, sensitivity, URL round-trip, and ADR generation, run in **CI**. | Must | BS §14; Charter §14.7 | T |
| NFR-MAINT-3 | Maintain a **model version distinct from the app (SemVer) version**. | Should | Charter §15 | I |
...
|---|---|---|
| FR-DATA-1 | **12 quality attributes**, each with id, EN/ID name & definition, ISO 25010 mapping, and an economic flag. | BS §3 |
| FR-DATA-2 | **≥12 (target 14) factors**, each with id, EN/ID label, three EN/ID level labels, EN/ID help, and a group. | BS §4 |
| FR-DATA-3 | A **factor→QA influence matrix** of non-zero influences (budget inverted). | BS §5 |
| FR-DATA-4 | **Five dimensions** with options, each option carrying a 12-value `qaFit` vector (1–5), educational metadata (EN/ID), and risks. | BS §6, §7, §9 |
| FR-DATA-5 | **Anti-pattern rules** (condition + severity + EN/ID message). | BS §10 |
| FR-DATA-6 | **Fitness-function templates** keyed to QAs (EN/ID). | BS §11 |
| FR-DATA-7 | **Scenario presets** and **migration paths**. | BS §12 |
| FR-DATA-8 | A defined **persisted-state schema** and a stable **URL-encoding** of it, both versioned by model version. | BS §2; FR-STATE-3 |
| FR-DATA-9 | A documented **custom-configuration JSON** schema for import/export. | BS §12 |
...
[Build Spec v3 §14](../specs/build-spec-v3.md); this SRS adopts them as its verification basis.
Key gates:
...
**Release gate (Charter [§11](../01-discovery-and-planning/discovery-and-planning.md#11-success-criteria--project-level-definition-of-done)):**
...
|---|---|---|---|
| FR-SHELL (modes, i18n, theme, presets, disclaimer) | §5, §7, §21 | §2, §12 | K2, K6 |
| FR-FACT (factors) | §5 | §4, §5 | K3 |
| FR-QA (priorities/utility tree) | §1, §5 | §3, §5 | — |
| FR-REC (5 dimensions + analysis) | §1, §5 | §6–§11 | K2, K6 |
| FR-OUT (ADR, report, CSV/JSON, share, config) | §5 | §12 | K4 |
| FR-STATE (persistence, reproducibility) | §15, §18 | §2 | — |
| FR-UI (mature states) | §11 | §12 | K2, K5 |
| FR-DATA (decision model) | §14.7 | §3–§12 | — |
| NFR-PERF | §4 | §1 | K3 |
| NFR-USE / A11Y / I18N | §5, §11 | §14 | K5, K6 |
| NFR-SEC / PRIV | §17, §18 | §2 | — |
| NFR-MAINT | §14.7, §15 | §13, §14 | K7 |
...
| OI-6 | Empirical-validation study design (deferred per Charter §5) | Owner | Out of MVP scope; tracked for v3.0 |
