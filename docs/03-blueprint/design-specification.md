...
| ADR-009 | **"Aurora Slate" visual design direction** — a full reskin *in place* over the existing React app (extends ADR-006's token contract with an aurora palette, an accent gradient, and a Space Grotesk display face; two signatures only — aurora background + the existing radar restyled). **The blueprint's proposed vanilla/MPA rewrite (its "ADR-000") is explicitly rejected:** ADR-003 (client-side) and ADR-004 (React) stand; the engine, the 21-architecture model, the seven-section Insights area, and all 99 tests are preserved. Visual source of truth: [`prototype-v2/preview-modern.html`](prototype-v2/preview-modern.html). Reconciliations (React SPA vs the greenfield-MPA blueprint): keep the `html.light` theme mechanism (not `[data-theme]`); keep Tailwind + `--color-*`/`--aa-*` naming (no blind rename); same-document (not cross-document) View Transitions; fold KPIs into the existing CI gates; PWA/service-worker deferred as an optional follow-up | Modern look without discarding a shipped, tested product; cost: a large restyle surface, mitigated by remapping existing token *values* so components inherit the palette. Accepted 2026-07-12 |
