...
export const ACADEMY_QUIZZES: QuizModule[] = [
  {
    ...
    questions: [
      ...
      {
        q: 'A layered deployment style is quick and familiar — its classic long-term risk is…',
        choices: ['Too much network latency', 'Eroding into a "big ball of mud" as the domain grows', 'It cannot use a database', 'It requires many teams'],
        answer: 1,
        explain: 'Without enforced boundaries, technical layers tangle as the domain grows — the erosion the Catalog and Review pages price in.',
  ...
  },
  {
    ...
    questions: [
      ...
      {
        q: 'Polyglot persistence means…',
        choices: ['Translating the schema into many languages', 'Using the right kind of database per access pattern — accepting the operational cost', 'One database vendor for everything', 'Storing data in JSON only'],
        answer: 1,
        explain: 'Sharply divergent access patterns can justify different stores (document, graph, key-value) — but every extra engine is a permanent operational bill.',
